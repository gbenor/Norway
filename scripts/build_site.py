...
route=[('Oslo','oslo.html'),('Holmenkollen','holmenkollen.html'),('Hemsedal','hemsedal.html'),('Borgund','fjord-country.html'),('Snow Road','fjord-country.html'),('Flåm','flam.html'),('Nærøyfjord','flam.html'),('Hardangervidda','hardanger.html'),('Trolltunga','trolltunga.html'),('Rosendal','rosendal.html'),('Bergen','bergen.html')]
...
def shell(title,body,slug,prev=None,next=None):
    ...
    route_html=''.join(f'<span class="{"current" if ((slug=="fjord-country" and r in ["Borgund","Snow Road"]) or (slug=="flam" and r in ["Flåm","Nærøyfjord"]) or (slug=="hardanger" and r=="Hardangervidda") or r.lower()==slug) else ""}">{r}</span>' for r, _href in route)
    nav=''
...
# home
home_highlights=[('Trolltunga','trolltunga.html','The defining hike above Ringedalsvatnet.','photos/day02/nordmarka/photo01.jpg'),('Flåm Railway','flam.html','A mountain railway that makes engineering feel cinematic.','photos/day02/holmenkollen/photo03.jpg'),('Snow Road','fjord-country.html','Aurlandsfjellet, snow walls and the descent toward the fjords.','photos/day02/nordmarka/photo02.jpg'),('Nærøyfjord','flam.html','A narrow fjord cruise through cliffs and waterfalls.','photos/day01/opera-house/photo03.jpg'),('Bergen','bergen.html','The rain-bright finale between mountains and sea.','photos/day01/fram-museum/photo01.jpg'),('Steinsdalsfossen','rosendal.html','A waterfall you can walk behind on the road to Bergen.','photos/day01/vigeland/photo02.jpg')]
home_body=f'''<section class="home-hero"><div class="home-hero-copy"><p class="eyebrow">Norway road trip journal · June 2026</p><h1>Lital & Gilad Explore Norway</h1><p>A Journey Through Fjords, Mountains and Norwegian Culture</p><div class="btns"><a class="btn" href="#origin">Begin the story</a><a class="btn ghost" href="#timeline">Explore the route</a></div></div><figure class="home-hero-photo"><img src="photos/day02/holmenkollen/photo01.jpg" alt="Norway mountain landscape from the trip"><figcaption>Selected trip photo for the journey opener</figcaption></figure></section><main id="main"><section class="wrap grid" id="origin"><div class="intro"><h2>Origin Story</h2><p>The idea for this trip did not begin as a classic holiday plan. It started because Lital had an EMDR conference in Oslo. Instead of treating the journey as a short professional trip, we decided to take advantage of being in Norway and continue into a full road trip across the country.</p><p>At first, the plan sounded simple: arrive in Oslo, see the famous fjords, experience something close to the classic “Norway in a Nutshell” route, and finish in Bergen. But very quickly the trip became much more than a list of attractions.</p></div><aside class="postcard-quote">“It often felt as though we were living inside a postcard.”</aside></section><section class="wrap" id="timeline"><h2>Route Overview</h2><div class="route-timeline">{''.join(f'<a href="{href}">{name}</a>' for name, href in route)}</div></section><section class="wrap"><h2>Trip Highlights</h2><div class="highlights">{''.join(f'<a class="feature" href="{href}"><img src="{img}" alt="{name} trip preview"><div class="feature-copy"><h3>{name}</h3><p>{desc}</p></div></a>' for name,href,desc,img in home_highlights)}</div></section><section class="wrap card"><h2>Photo naming rules</h2><p>Add <code>photo01.jpg</code>, <code>photo02.jpg</code>, <code>photo03.jpg</code> and onward into each gallery folder. The website tries those names automatically and shows “Photos coming soon.” when a folder is empty. No HTML edits are required.</p></section></main>'''
